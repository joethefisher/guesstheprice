/* global React */
// src/summary.jsx — End-of-game summary screen
    ...
    <div style={{
      ...
      <div style={{ position: "relative", zIndex: 2, padding: "32px 56px 56px", maxWidth: 1320, margin: "0 auto" }}>
        ...
        <div style={{
          ...
          <div>
            <div className="eyebrow" style={{ marginBottom: 14 }}>Round complete · 10 of 10</div>
            <h1 className="display" style={{
              margin: 0, fontSize: "clamp(72px, 9vw, 132px)",
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            <BigStat label="Average accuracy" value={avgAcc + "%"} note={
              ...
              avgAcc >= 60 ? "Solid week." :
              avgAcc >= 40 ? "Decent vibes." :
                             "There's always tomorrow."
            }/>
            <BigStat label="Best market" value={best.listing.city} note={
              `+${best.points} on the ${best.listing.neighborhood} ${best.listing.beds === 0 ? "studio" : "home"}`
            }/>
          </div>
        </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            borderRadius: 22, padding: "32px 32px 28px",
            ...
            <p style={{ color: "rgba(247,244,238,0.7)", fontSize: 14.5, margin: 0, lineHeight: 1.5 }}>
              Daily challenge resets at midnight local. 5 hand-picked homes,
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 8 }}>
              <button className="btn" onClick={onDaily} style={{
                ...
                Daily challenge — May 9 <Icon.Arrow size={18}/>
              </button>
              <button className="btn" onClick={onPlayAgain} style={{
                ...
                Play another 10 <Icon.Arrow size={18}/>
              </button>
            </div>
          </div>
        </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            backgroundImage: `url(${worst.listing.photos[0]})`,
            backgroundSize: "cover", backgroundPosition: "center",
            background: worst.listing.bandColor,
          }}/>
          <div style={{ flex: 1 }}>
            <span className="caption" style={{ color: "var(--flag)", fontSize: 10 }}>WORST GUESS · ROUND {rounds.indexOf(worst) + 1}</span>
            <div style={{
              fontFamily: "var(--display)", fontStyle: "italic", fontSize: 22,
              margin: "6px 0 4px", letterSpacing: "-0.01em",
            }}>
              {worst.listing.neighborhood}, {worst.listing.city} got the better of you.
            </div>
            <div style={{ fontSize: 13.5, color: "var(--ink-mute)" }}>
              ...
              It was <span className="tnum" style={{ fontWeight: 600 }}>${worst.listing.price.toLocaleString()}</span>.
...
function BigStat({ label, value, note }) {
  return (
    <div style={{
      padding: "18px 22px", background: "var(--cream)", borderRadius: 16,
    }}>
...
function RoundCard({ idx, r, accent, isBest, isWorst }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        height: 90,
        backgroundImage: `url(${r.listing.photos[0]})`,
        backgroundColor: r.listing.bandColor,
        backgroundSize: "cover", backgroundPosition: "center",
      }}/>
      <div style={{ padding: "10px 12px 12px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 6 }}>
          <span className="caption tnum" style={{ fontSize: 9, color: "var(--ink-mute)" }}>R{String(idx).padStart(2,"0")}</span>
          <span className="tnum" style={{ fontSize: 11, fontWeight: 600, color: tier.color }}>
            {Math.round(r.accuracy)}%
          </span>
        </div>
        <div style={{
          fontFamily: "var(--display)", fontStyle: "italic", fontSize: 14, fontWeight: 500,
          letterSpacing: "-0.01em", margin: "0 0 6px", color: "var(--ink)",
        }}>{r.listing.neighborhood}</div>
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "var(--ink-mute)" }} className="tnum">
          ...
          <span style={{ color: "var(--ink)", fontWeight: 600 }}>{window.fmtShortPrice(r.listing.price)}</span>
        </div>
      </div>
    </div>
  );
...
function ShareCard({ rounds, avgAcc, tiles, accent, dateStr }) {
  ...
  function copy() {
    const text = `Pricetag · ${dateStr}\n${tiles}\n${avgAcc}% accurate · ${rounds.length} rounds\nplay.pricetag.app`;
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ fontSize: 28, letterSpacing: "0.04em", lineHeight: 1.2, marginBottom: 8 }}>
          {tiles}
        </div>
        <div className="tnum" style={{ color: "var(--ink)", fontWeight: 600 }}>
          {avgAcc}% accurate · 4-day streak
        </div>
        <div style={{ color: "var(--ink-quiet)", fontSize: 11, marginTop: 4 }}>
          play.pricetag.app
        </div>
      </div>
